...
class MultiDMatern32(cov.CovarianceFunction):
    ...
    def __init__(self, theta, X=None, Y=None):
        ...
        if (np.min(self.theta) <= 0.0):
            warnings.warn("Illegal hyperparameters in the" +
                          " initialization of MultiDMatern32.")
    ...
    def covfunc(self):
        ...
        covariance = sigmaf**2 * (1.0 + sqrt(3.) * rl) * exp(-sqrt(3.) * rl)
        return covariance
    ...
    def gradcovfunc(self):
        ...
        dk_dsigmaf = float(2 * sigmaf * (1.0 + sqrt(3.) * rl) *
                           exp(-sqrt(3.) * rl))
        grad[0] = dk_dsigmaf
        grad[1:] = 3 * sigmaf**2 * r[:]**2/l[:]**3 * exp(-sqrt(3.) * rl)
        return grad
