...
class Matern32(cov.CovarianceFunction):
    ...
    def __init__(self, theta, X=None, Y=None):
        ...
        if self.theta[0] <= 0.0 or self.theta[1] < 0.0:
            warnings.warn(
                "Illegal hyperparameters in the" + " initialization of Matern32"
            )
    ...
    def covfunc(self):
        ...
        covariance = sigmaf**2 * (1.0 + sqrt(3.0) * r / l) * exp(-sqrt(3.0) * r / l)
        return covariance
    ...
    def gradcovfunc(self):
        ...
        r = sqrt(np.sum((self.x1 - self.x2) ** 2))
        dk_dsigmaf = float(
            2 * sigmaf * (1.0 + sqrt(3.0) * r / l) * exp(-sqrt(3.0) * r / l)
        )
        dk_dl = float(3 * sigmaf**2 / l**3 * r**2 * exp(-sqrt(3.0) * r / l))
    ...
    def dcovfunc(self):
        ...
        dcov = 3 * sigmaf**2 / l**2 * exp(-sqrt(3.0) * r / l) * (self.x1 - self.x2)
        return float(dcov)
    ...
    def d2covfunc(self):
        raise RuntimeError(
            "Matern32 cannot reconstruct second (or higher)" + " derivatives."
        )
    ...
    def dgradcovfunc(self):
        ...
        dgrad_s = float(
            6 * sigmaf / l**2 * exp(-sqrt(3.0) * r / l) * (self.x1 - self.x2)
        )
        dgrad_l = float(
            3
            * sigmaf**2
    ...
    def ddgradcovfunc(self):
        ...
        r = float(sqrt((self.x1 - self.x2) ** 2))
        ddgrad_s = float(
            6 * sigmaf / l**4 * exp(-sqrt(3.0) * r / l) * (1.0 - sqrt(3.0) * r / l)
        )
        ddgrad_l = float(
            3
            * sigmaf**2
