...
class Matern72(cov.CovarianceFunction):
    ...
    def __init__(self, theta, X=None, Y=None):
        ...
        if self.theta[0] <= 0.0 or self.theta[1] < 0.0:
            warnings.warn(
                "Illegal hyperparameters in the" + " initialization of Matern72."
            )
    ...
    def ddcovfunc(self):
        ...
        dcov = (
            ...
            * sigmaf**2
            / (15.0 * l**2)
            * erl
            * (3.0 + 3.0 * sqrt(7.0) * rl - 7.0 * sqrt(7.0) * rl**3)
        )
        return float(dcov)
    ...
    def d2d2covfunc(self):
        ...
        dcov = (
            ...
            * sigmaf**2
            / (15.0 * l**4)
            * erl
            * (3.0 + 3.0 * sqrt(7.0) * rl - 42.0 * rl**2 + 7.0 * sqrt(7.0) * rl**3)
        )
        return float(dcov)
    ...
    def d3d3covfunc(self):
        ...
        dcov = (
            ...
            * sigmaf**2
            / (15.0 * l**6)
            * erl
            * (15.0 - 33 * sqrt(7.0) * rl + 84.0 * rl**2 - 7 * sqrt(7.0) * rl**3)
        )
        return float(dcov)
    ...
    def dgradcovfunc(self):
        ...
        dgrad_l = float(
            ...
            * sigmaf**2
            / (15.0 * l**3)
            * erl
            * (-6.0 - 6 * sqrt(7.0) * rl - 7.0 * rl**2 + 7 * sqrt(7.0) * rl**3)
            * (self.x1 - self.x2)
        )
