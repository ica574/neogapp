...
class Matern52(cov.CovarianceFunction):
    ...
    def __init__(self, theta, X=None, Y=None):
        ...
        if self.theta[0] <= 0.0 or self.theta[1] < 0.0:
            warnings.warn(
                "Illegal hyperparameters in the" + " initialization of Matern52."
            )
    ...
    def d2d3covfunc(self):
        raise RuntimeError(
            "Error: Matern52 cannot reconstruct third " + "(or higher) derivatives"
        )
