...
class MultiDMatern52(cov.CovarianceFunction):
    ...
    def __init__(self, theta, X=None, Y=None):
        ...
        if (np.min(self.theta) <= 0.0):
            warnings.warn("Illegal hyperparameters in the" +
                          " initialization of MultiDMatern52.")
    ...
    def covfunc(self):
        ...
        covariance = sigmaf**2 * (1 + sqrt(5.) * rl + 5./3. * (rl)**2) * \
            exp(-sqrt(5.) * rl)
        return covariance
    ...
    def gradcovfunc(self):
        ...
        dk_dsigmaf = float(2 * sigmaf * (1 + sqrt(5.) * rl + 5./3. *
                                         (rl)**2) * exp(-sqrt(5.) * rl))
        grad[0] = dk_dsigmaf
        grad[1:] = 5/3. * sigmaf**2 * r[:]**2/l[:]**3 * (1 + sqrt(5.) * rl) * \
            exp(-sqrt(5.) * rl)
        return grad
