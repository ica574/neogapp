...
class DGaussianProcess(gp.GaussianProcess):
    def __init__(
        ...
        assert (
            shape(X) == () or shape(X) == (len(X),) or shape(X) == (len(X), 1)
    ...
    def log_likelihood(
        ...
        if dX != "False" and dY != "False" and dSigma != "False":
            ...
            if dX is not None or dY is not None or dSigma is not None:
                ...
                warnings.warn(
                    "dX, dY and dSigma have to be changed "
    ...
    def fdgp(self, unpack):
        ...
        if (
            ...
            or self.covf.scaletrain == "True"
            or self.covf.dscaletrain == "True"
        ):
            self.hypertrain()
        # reconstruct f'(x)
        ...
        self.dfmean = dfmean
        self.dfstd = dfstd
        self.dreconstruction = concatenate(
            (
                self.Xstar,
    ...
    def fd2gp(self, unpack):
        ...
        self.d2fstd = d2fstd
        self.d2reconstruction = concatenate(
            (
                self.Xstar,
    ...
    def fd3gp(self, unpack):
        ...
        self.d3fstd = d3fstd
        self.d3reconstruction = concatenate(
            (
                self.Xstar,
    ...
    def dm_grad_covariance(self):
        ...
        if self.covf.dscaletrain == "True":
            ...
            for i in range(dn):
                graddscale[0, n + i, n + i] = 2 * self.covf.dscale * self.dSigma[i, i]
    ...
    def dm_nlog_likelihood(self):
        ...
        if self.prior is not None:
            ...
            if priorp < 0.0:
                warnings.warn(
                    "Invalid prior. Negative prior values "
        ...
        if self.dmalpha is None:
            ...
            logp = -(
                -0.5 * dot(YdY, self.dmalpha)
                - np.sum(log(diagonal(self.dmL)))
                - (self.n + self.dn) / 2 * log(2 * pi)
                + priorlogp
            )
        return logp
    ...
    def dm_hypertrain(self):
        ...
        if self.grad == "True":
            ...
            if self.covf.scaletrain == "False" and self.covf.dscaletrain == "False":
                ...
                # scale and some hyperparameters will be trained
                    ...
                    if self.covf.dscaletrain == "True":
                        ...
                        if self.covf.scaletrain == "True":
                            ...
                            bound.append(bounds[len(initheta)])
                    # array of the initial values of these hyperparameters
                    initheta_s = append(initheta, (self.scale, self.covf.dscale))
                    ...
                    def logpfunc(th_s):
                        ...
                        (logp, gradlogp) = self.dm_grad_nlog_likelihood()
                        gradlogp = append(
                            take(gradlogp, indices), gradlogp[len(initheta) :]
                        )
                        return (logp, gradlogp)
            ...
            if self.covf.scaletrain == "False" and self.covf.dscaletrain == "False":
                ...
                # only scale will be trained. hyperparameters are fixed
                    ...
                    sc = opt.fmin_cobyla(
                        logpfunc, sc, constraints, args=(), consargs=()
                    )
                    if self.covf.scaletrain == "True":
                        sds[0] = sc[0]
                    if self.covf.dscaletrain == "True":
    ...
    def dm_dgp(
        ...
        if self.covf.scale is None and self.covf.dscale is None:
            ...
            if unpack == "False":
                ...
                return (self.Xstar, self.dfmean, self.dfstd, self.covf.theta, sds)
    ...
    def dm_d2gp(
        ...
        if self.covf.scale is None and self.covf.dscale is None:
            ...
            if unpack == "False":
                ...
                return (self.Xstar, self.d2fmean, self.d2fstd, self.covf.theta, sds)
    ...
    def dm_d3gp(
        ...
        if self.covf.scale is None and self.covf.dscale is None:
            ...
            if unpack == "False":
                ...
                return (self.Xstar, self.d3fmean, self.d3fstd, self.covf.theta, sds)
