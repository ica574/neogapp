...
def mcmc_log_likelihood(
    th, sc0, scl, X, Y_mu, Sigma, dX, dY_dmu, dSigma, covfunction, prior, priorargs
):
    try:
        if np.min(th) < 0.0:
            return np.NINF
    ...
    try:
        if fcovj != None:
            ...
            for i in range(1, len(recj)):
                rs = random.multivariate_normal(
                    recj[i, 0, :], fcovj[i, :, :], k[i] * nsample
                )
            ...
            for i in range(len(recj)):
                if recj[i, 1, 0] > 0:
                    rarr = concatenate(
                        (
                            rarr,
                            random.normal(recj[i, 0, 0], recj[i, 1, 0], k[i] * nsample),
                        )
                    )
                else:
                    rarr = concatenate((rarr, recj[i, 0, 0] * ones(k[i] * nsample)))
...
class MCMCDGaussianProcess(dgp.DGaussianProcess):
    def __init__(
        ...
        if scale0 != None:
            ...
            if self.scl == 2 and dX is None:
                ...
                self.scl = 1
                warnings.warn(
                    "scale0 is two-dimensional, but dX=None. "
                    + "Second dimension of scale0 will be ignored."
                )
            self.pos = concatenate(
                (theta0, reshape(scale0, (len(scale0), self.scl))), axis=1
            )
        else:
            self.pos = theta0
            self.sc0 = False
    ...
    def serialrec(self, redpossample, k):
        ...
        for j in range(self.nstar):
            if nrl > 1:
                rarr = random.multivariate_normal(
                    rec[0, j, 0, :], fcov[0, j, :, :], k[0] * self.nsample
                )
                for i in range(1, len(rec)):
                    rs = random.multivariate_normal(
                        rec[i, j, 0, :], fcov[i, j, :, :], k[i] * self.nsample
                    )
                    rarr = concatenate((rarr, rs))
                reconstrarr[j, :, :] = rarr[:, :]
            else:
                ...
                for i in range(len(rec)):
                    if rec[i, j, 1, 0] > 0:
                        ...
                        rarr = concatenate(
                            (rarr, rec[i, j, 0, 0] * ones(k[i] * self.nsample))
                        )
    ...
    def parallelrec(self, redpossample, k):
        ...
        if nrl > 1:
            recon = [
                pool.apply_async(
                    recarray, (j, rec[:, j, :, :], fcov[:, j, :, :], k, self.nsample)
                )
                for j in range(self.nstar)
            ]
        else:
            recon = [
                pool.apply_async(recarray, (j, rec[:, j, :, :], None, k, self.nsample))
                for j in range(self.nstar)
            ]
        if nrl > 1:
            for r in recon:
